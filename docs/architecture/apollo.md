...
| `apollo/parser/` | `parser_llm.py`, `prompt_builder.py`, `extraction_schema.py`, `graph_context.py`, `edge_resolver.py` | `parse_utterance()`: ONE strict-`json_schema` GPT-4o call emitting typed Nodes AND all four typed Edges (PRECEDES/USES/SCOPES/DEPENDS_ON) with explicit/inferred provenance; optional `graph_context` (default None — `chat.py` unchanged until WU-2B wires it) lets edges link across turns. When no context is supplied AND the model emits no edges, the deterministic within-turn USES/PRECEDES derivation is the fallback (today's behavior). Triviality detection unchanged (length floor, ACK list, math-char regex, then cheap-LLM classifier at threshold 0.6). `extraction_schema.build_extraction_schema()` = the strict OpenAI json_schema (nodes + typed edges + provenance). `graph_context.GraphContext` = frozen prior-attempt node summaries threaded in by WU-2B. `edge_resolver.resolve_typed_edges()` = ref resolution + EDGE_ALLOWED_PAIRS pre-check + provenance tagging, rejecting bad edges by logging `parser_edge_rejected` (no silent drop, no raise). `prompt_builder.py` substitutes `{{concept_name}}` into the concept's `parser_prompt_template.md` (v1: parser captures the student's own form; canonical-symbol slots removed). |
...
3. Parse: `parse_utterance` (ONE strict-json_schema GPT-4o call, temp 0.0) → typed nodes (`node_id = "stu_" + uuid12`) + all four typed edges (provenance-tagged), cross-turn when a `graph_context` is supplied. v1 chat passes no `graph_context`, so this behaves like the within-turn-only parser; the deterministic USES (from `uses_equation_ordinals`) + PRECEDES (consecutive procedure steps) derivation is the no-context fallback used when the model emits no edges. (WU-2B will build a `GraphContext` from the live attempt graph and thread it in, and fix `store.write_edges`'s silent-drop.)
...
- `ai-ta-backend/docs/superpowers/plans/2026-06-15-apollo-kg-wu2a-typed-edge-extraction.md` — WU-2A plan (this change: context-aware one-call typed-edge extraction + explicit/inferred provenance, parser-only; WU-2B wires `graph_context` + store).
