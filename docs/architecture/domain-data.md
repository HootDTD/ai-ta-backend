...
| `knowledge/teacher_weekly.py` | `TeacherWeeklyStorage` — teacher weekly uploads (Supabase Storage + DB job queue with leases), current-week + retrieval-weight controls, week-based document activation |
...
- `TeacherUpload` (`teacher_uploads`): `week`, `kind` (notes/slides), `status` (queued/processing/ready/failed/superseded), `storage_key`, `doc_id` FK → aita_documents SET NULL, `is_latest`, `ocr_provider`, `ocr_summary` JSONB, `artifact_manifest` JSONB, `metadata` JSONB.
