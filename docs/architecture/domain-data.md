...
references to its job and rejection tables; those legacy tables are now
covered by the DB-16 teardown below. Migration 030 remains documented below as
...
DB-16 (2026-07-21) authored the delayed, separately-gated legacy `public`
schema teardown at `scripts/db/remove_legacy_public_schema.sql` -- deliberately
**not** under `supabase/migrations/` (which `supabase db reset` and
`node scripts/db/reset-local.mjs` apply unconditionally) so this one-shot,
irreversible DROP sequence stays a human-run step taken only after the
production soak plan section 8.3 requires. It drops the seven approved
no-copy tables (plan section 4's six plus `apollo_kg_negotiations` per
amendment A6) in their stated order, then the remaining 35 superseded legacy
tables in full child-first FK-safe order derived from the FK web in
`.planning/cleanup/findings/c-db-usage.md`, ending with `aita_search_spaces`
(21 inbound references) last. Authoring it surfaced two tables amendment A7
also orphaned but never added to the no-copy list --
`apollo_graph_comparison_runs`/`apollo_graph_comparison_findings`, whose ORM
models DB-14 already deleted with zero remaining importers -- so the teardown
gates those the same way. Every precondition (the 7+2 no-copy row-count
ceilings, and a full count+checksum reconciliation of all 33 genuinely copied
tables, reusing `scripts/db/reconcile_copy.sql`'s `assert_copy_mapping`
technique verbatim) runs inside the same transaction as the drops, so a
tampered precondition aborts before a single `DROP TABLE` executes; none of
the drops use `CASCADE`. `tests/database/test_remove_legacy_public_schema.py`
rehearses all of this on Testcontainers Postgres (a clean run, a deleted
copied row, and an out-of-band-missing no-copy table). DB-16 also deleted the
two now-obsolete legacy-migration test pins for tables it drops --
`tests/database/test_apollo_autoprovisioning_migration.py` and
`tests/database/test_apollo_misconception_obs_source_domain_migration.py` --
and removed the superseded hand-created `public.fetch_items`/`fts_count`/
`hybrid_search` functions (never captured in a migration file; see
`.planning/cleanup/inputs/db-preflight.md`), superseded by DB-06's
`internal.*` equivalents. This script has not been applied anywhere; the
legacy tables remain physically present until a human runs it post-soak.
...
DB-13 (2026-07-21) retargets the Layer-1/Layer-3 Apollo learner-model ORM
(`apollo/persistence/models.py`) onto the DB-04 target DDL: `LearnerEntity`
(`app.learner_entities`), `EntityPrereq` (`internal.entity_prerequisites`),
`LearnerState` (`app.learner_state`), and `MasteryEvent` (`app.mastery_events`)
replace the legacy `KGEntity`/`apollo_kg_entities` naming. `course_id` is a new
denormalized `NOT NULL` column on `LearnerEntity`/`EntityPrereq`, threaded
through every mint/seed caller. `misconception_code` is dropped from
`LearnerState`/`MasteryEvent` (misconceptions live on `MasteryEvent.event_kind`
and `LearnerEntity.kind`), and `aliases`/`evidence_node_ids` become `TEXT[]`.
Per MIG-AMEND's A6 ruling, `apollo_kg_negotiations` is dropped with no target
table — DB-13 removes the `KGNegotiation` ORM model and
`KGStore._log_negotiation`'s audit-row write; the challenge/paraphrase/skip
...
- `ChatSessionSnippet` (`chat_session_snippets`): composite PK (`chat_session_id`, `chunk_id`), `original_score`, `first_seen_turn`, `last_used_turn`, `snippet_payload` JSONB — citation cache enabling retrieval-mode NONE/AUGMENT to skip pgvector. Read/written by `chats/bundle_cache.py` on the always-on retrieval-mode paths (the router master switch was removed in the 2026-07 flag reset); the visible-docs fingerprint lives in `chat_sessions.meta["bundle_cache"]`. (`chat_sessions.topic_centroid_vector` remains unused — reserved for a future embedding fast-path.)
