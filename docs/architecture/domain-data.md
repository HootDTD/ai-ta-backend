...
- `ChatTurn` (`chat_turns`): `chat_session_id` FK, `turn_index`, `turn_id`, `role`, `content`, `model`, `tool_name`, `tool_inputs` JSONB, `attachments` JSONB, `citations` JSONB, `keywords` JSONB (§10 RQ5 hedge, migration 029 — write-only ≤8 concept terms from `extract_and_filter_keywords`, persisted for offline class-level backfill; defaults to `[]`; NO read path in v1, and the production `server.py` wire-up that fills it from `bundle.found_terms` is a documented follow-on — until it lands the column round-trips but the live `/ask` path writes `[]`. Offline backfill reads it off `role='assistant'` rows).
