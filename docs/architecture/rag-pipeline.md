...
6. **Hybrid retrieval** — `retrieval/pipeline.py:retrieve_for_question`: builds `combined_query = question + " " + top-6 keywords` (keywords are appended hints, never substitutes), then `AITAHybridSearchRetriever.hybrid_search(combined_query, top_k=top_k*3)`: embeds the query (`indexing/document_embedder.py:embed_text`, `text-embedding-3-large` @ 3072 dims, LRU-cached), runs two CTEs over `aita_chunks` joined to `aita_documents` filtered by `document_visibility.active_document_conditions` (status `ready` + weekly notes/slides gated to `TeacherUpload.week <= current_week`), each limited to `top_k*5` candidates, FULL OUTER JOINed and scored `1/(60+sem_rank) + 1/(60+kw_rank)`.
