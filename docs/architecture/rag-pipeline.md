...
5. **Keyword extraction (query expansion)** — `server.py:_ask_pgvector` (line ~1433) calls `ai/main_ai.py:extract_and_filter_keywords` — ONE LLM call (concept_extraction prompt) returning ≤8 ranked textbook-index-style terms. Failure → empty keyword list (retrieval still runs on the raw question). As of WU-5B4 (§10 RQ5 hedge) the same ≤8 extracted concept terms — carried unchanged on `ResearchBundle.found_terms` (`ai/orchestrator.py:_retrieve`) — are ALSO PERSISTED to `chat_turns.keywords` at the assistant-turn write (migration 029, owned by `domain-data.md`), a **write-only** offline-backfill signal. This does NOT change retrieval, the LLM call, or any score (the keywords remain hints only); the orchestrator surfaces the already-computed list, it is not recomputed. **WU-5B5 landed the LIVE wire-up:** the production `/ask` handler now passes the keyword list into `append_turn(keywords=...)` on the assistant turn via `_append_assistant_turn_and_refresh(..., keywords=_keywords_from_bundle(bundle))` (step 14) on BOTH happy paths (`/ask` non-streaming + `/ask/stream`). `server.py:_keywords_from_bundle(bundle) -> List[str]` is a small pure extractor that robustly reads `bundle.found_terms` (top-level) or `bundle.metadata.found_terms` (fallback), returning `[]` for a `None`/AUGMENT/cache bundle that carries none; it returns a fresh `[str(t) for t in terms]` copy and does NOT re-cap at 8 (the ≤8 bound is upstream here). The error path and cache/`None` bundles still write `[]`. There is still NO read path in v1.
...
14. **Response** — `server.py:_structured_citations_from_bundle` builds citation objects; answer + citations persisted as assistant chat turn (`_append_assistant_turn_and_refresh`/`_async`, keyword-only `keywords` param forwarded into `append_turn(keywords=...)`) and returned (JSON for `/ask`, SSE `answer` event for `/ask/stream`). The assistant turn ALSO writes `chat_turns.keywords = _keywords_from_bundle(bundle)` (the per-`/ask` ≤8-term `bundle.found_terms`, §10 RQ5 hedge) — write-only, no behavior change to answer/citations; the streaming **error** path and the **user** turn (`_load_memory_and_append_user_turn`) carry no keywords (`[]`).
...
- **Keywords are hints, never substitutes** — `retrieve_for_question` appends ≤6 keywords to the original question so bad extraction can't kill retrieval. The original question always anchors semantic search. As of WU-5B4 they are additionally persisted (write-only) to `chat_turns.keywords` for the §10 RQ5 offline backfill, with no effect on retrieval; WU-5B5 wired this into the LIVE `/ask` path (`_keywords_from_bundle(bundle)` → `append_turn(keywords=...)` on the assistant turn of both happy paths). Still no read path.
