...
- `embed_text(text) -> list[float]` (indexing/document_embedder.py) — the only embedding entry point in this pipeline.
...
6. **Chunk + embed**: `teacher_weekly.py:_index_existing_upload_async` → `indexing/indexing_service.py:prepare_for_indexing` → `index_from_items` → `indexing/document_chunker.py:items_to_chunk_texts` (1 Item = 1 chunk) → `indexing/document_embedder.py:embed_text` once per chunk plus one document-level embedding over the first 2000 chars of body/heading/ocr text.
