...
**Goal:** Ship the behavior-preserving foundation the WU-5B3a drain state machine will stand on — migration 028 (dead-letter + backoff columns + partial index), the `retired rerun input builder` shared input-builder, `KGStore.read_node_graded_at`, and `LearnerUpdateUnreconstructableError` — each independently green on real infra, with `done.py` refactored to call the builder (byte-identical).
...
provides:
  ...
  - `apollo/handlers/done_inputs.py::retired rerun input builder(db, neo, *, attempt, sess) -> RerunInputs` (+ frozen `RerunInputs` dataclass)
...
depends_on:
  ...
  - WU-5A2 (the live `done.py` shadow-chain + `retired graph simulation`/`run_learner_update` it preserves)
...
5. **`done.py` refactor** extracts its shadow-input assembly into `retired rerun input builder` and calls it — so the live path and the future janitor share ONE builder (no drift). Behavior-preserving: the existing route tests stay green.
...
- NO call to `retired rerun input builder` from anything except `done.py` in this unit. The janitor caller is 5B3a-1.
...
- [ ] **None — neighborhood is clean for this unit's blast radius.** Scanned the one-ring-out change path:
  ...
  - `done.py` function size: `handle_done` is ~190 lines but the extraction REDUCES it (moves the ~10-line shadow-input assembly out). `retired rerun input builder` lands as a small focused module (<60 lines). No file approaches the 800-line ceiling. No WMC/responsibility threshold tripped.
...
- Verify (fan-in sanity, run before coding): `grep -rn "learner_update_attempts\|learner_update_failed_permanently\|read_node_graded_at\|retired rerun input builder" apollo/ database/ tests/` — expect ZERO hits (nothing references the new surface yet).
...
- **Shadow-input assembly to extract:** `apollo/handlers/done.py:402-413` — inside `if _graph_sim_shadow_enabled():`, the `problem_payload = await _find_problem_payload(db, concept_id=sess.concept_id, problem_code=problem.id)` read + the `old_rubric=rubric` pass into `retired graph simulation`. `_find_problem_payload` is at `done.py:229-250`; `min_parser_confidence_of` at `apollo/grading/abstention.py:70-76`.
...
# pre-flight (WU-5B3a retired rerun input builder) raises with exactly one of these.
...
class LearnerUpdateUnreconstructableError(ApolloError):
    ...
    transient failure. Raised by `retired rerun input builder`'s pre-flight BEFORE any LLM
...
No FastAPI handler is registered in this unit — this error never reaches an HTTP boundary (it is raised inside the offline janitor path; `retired rerun input builder` is only called by `done.py` in this unit, where the inputs are ALWAYS reconstructable because Done just wrote them, so it cannot fire on the live path). Registering a handler is a 5B3a-1/5B3b concern only if the janitor ever surfaces it.
...
## New shared input-builder (done_inputs.retired rerun input builder)

**File:** `apollo/handlers/done_inputs.py` (NEW, ~55 lines). The single source of truth for re-assembling the inputs `retired graph simulation` consumes. Both `done.py` (this unit) and the janitor (5B3a-1) call it.
...
async def retired rerun input builder(
    db: AsyncSession,
    neo: Neo4jClient,
    *,
...
**File:** `apollo/handlers/done.py` (EDIT). The refactor must be BEHAVIOR-PRESERVING — the only observable change is that the shadow `problem_payload` + `old_rubric` come through `retired rerun input builder` instead of being assembled inline. The student-facing grade, XP, retention stamp, and ALL flag semantics are untouched.

**Constraint that makes this safe and minimal:** the live `done.py` already holds `student_graph` (`= pre_freeze_graph`, `:288`), `rubric` (computed at `:304`), and reads `problem_payload` inline at `:402-405`. `retired rerun input builder` re-derives ALL of these. To keep the refactor a true single-source-of-truth extraction WITHOUT changing the live grade, the cleanest behavior-preserving move is:
...
- Inside the `if _graph_sim_shadow_enabled():` block, replace the inline `problem_payload = await _find_problem_payload(db, concept_id=sess.concept_id, problem_code=problem.id)` (`:403-405`) with a call to the shared builder that keys on `attempt.problem_id`:
  ```python
  rerun = await retired rerun input builder(db, neo, attempt=attempt, sess=sess)
...
**Net:** `done.py`'s observable behavior with every flag in its default (OFF) state stays byte-identical (`test_done_shadow_route_postgres.py` me==0/ls==0 guard); with SHADOW+LAYER3 ON the persisted state stays identical (`test_done_layer3_route_postgres.py`). The single new call is `retired rerun input builder` supplying `problem_payload`. Add `from apollo.handlers.done_inputs import retired rerun input builder` to the imports.

**Alternative the executor MAY take instead (allowed):** if relocating `_find_problem_payload` causes import churn the executor judges riskier than the win, keep `_find_problem_payload` in `done.py`, have `done_inputs.py` import it from `done.py` ONLY IF that does not create a circular import (it will, since `done.py` imports `retired rerun input builder`) — so the relocation IS the recommended path. Do not duplicate the function in both modules (that violates single-source-of-truth, the entire reason this unit exists).
...
- `apollo.handlers.done_inputs.retired rerun input builder(db: AsyncSession, neo: Neo4jClient, *, attempt: ProblemAttempt, sess: ApolloSession) -> RerunInputs`.
...
4. **`retired rerun input builder` (RED→GREEN, real PG + real Neo4j or stubbed Neo4j per test).**
   - Write `apollo/handlers/tests/test_retired rerun input builder.py` (the full builder test list below). The wrong-problem multi-problem regression is MANDATORY and is the #1 guard. Run → RED.
   - Create `apollo/handlers/done_inputs.py` (relocate `_find_problem_payload`, add `RerunInputs` + `retired rerun input builder`). → GREEN.
...
### D. `retired rerun input builder` — `apollo/handlers/tests/test_retired rerun input builder.py` (REAL PG via `db_session` + Neo4j stubbed or real per test)
...
12. **`test_retired rerun input builder_keys_on_attempt_problem_id_not_current_problem_id` (THE #1 GUARD — MANDATORY, MULTI-PROBLEM).** Seed a course with TWO problems (codes `p_a`, `p_b`). Create a session, an attempt for `p_a` (with `diagnostic_report={"rubric": {"overall": {...}}, ...}`), then ADVANCE the session: set `sess.current_problem_id = "p_b"` (simulating `next.py:94` after the student clicked Next). Stub `read_graph`→a graph, `read_node_graded_at`→`{"n1": "<iso>"}`. Call `retired rerun input builder(db, neo, attempt=<p_a attempt>, sess=sess)`. Assert `result.problem_payload` is `p_a`'s payload (NOT `p_b`'s) — i.e. the builder rebuilt the OLD problem the pending attempt belongs to, even though `current_problem_id` now points at `p_b`. **A single-problem fixture would PASS even with the wrong key — this two-problem fixture is the only thing that catches the make-or-break bug.** Assert the payloads differ between `p_a`/`p_b` first (fixture sanity) so the test can't trivially pass.
13. `test_retired rerun input builder_happy_path_assembles_all_fields` — single attempt with a well-formed `diagnostic_report`; stub `read_graph`→a 1-node graph (`parser_confidence=0.95`), `read_node_graded_at`→`{"n1":"2026-06-18T00:00:02+00:00"}`. Assert `RerunInputs` is frozen, `problem_payload` == the seeded payload, `old_rubric` == `diagnostic_report["rubric"]`, `student_graph` == the stubbed graph, `parser_confidence == 0.95` (= `min_parser_confidence_of`), `graded_at_iso == "2026-06-18T00:00:02+00:00"`.
14. `test_retired rerun input builder_dead_letters_when_diagnostic_report_none` — attempt with `diagnostic_report=None`; assert `retired rerun input builder` raises `LearnerUpdateUnreconstructableError` with `reason="diagnostic_report_missing"`, and assert (via an `AsyncMock` spy on `read_graph`/the absence of any grading call) that NO LLM/grading is reached — the raise happens at the pre-flight. (Use `pytest.raises` and assert `.reason`.)
15. `test_retired rerun input builder_dead_letters_when_rubric_missing` — `diagnostic_report={"narrative": "x", "coverage": {}}` (no `"rubric"` key) → raises with `reason="rubric_missing"`. Plus a variant: `diagnostic_report={"rubric": {"no_overall": 1}}` (rubric present but lacks `"overall"`) → also `reason="rubric_missing"` (guards the `calibration.py:88,93` unconditional `["overall"]` deref).
16. `test_retired rerun input builder_dead_letters_when_graded_at_empty` — well-formed `diagnostic_report`, but stub `read_node_graded_at`→`{}` (empty subgraph) → raises with `reason="graded_at_missing"`. The empty-Neo4j-subgraph terminal trigger.
...
- [ ] Fan-in sanity (before coding): `grep -rn "learner_update_attempts\|read_node_graded_at\|retired rerun input builder" apollo/ database/ tests/` — expect zero hits initially.
...
- [ ] Error + builder unit/integration: `.venv/Scripts/python.exe -m pytest apollo/persistence/tests/test_learner_update_unreconstructable_error.py apollo/handlers/tests/test_retired rerun input builder.py -v` — expect all PASS, including the multi-problem wrong-problem regression (test #12).
...
5. **Main data flows — step 8 (`:121`):** append a sentence: "**WU-5B3a-0** extracts the shadow-input assembly into `done_inputs.retired rerun input builder` (shared with the future janitor); `done.py` now sources the shadow `problem_payload` through it (keyed on `attempt.problem_id`), behavior-preserving with all flags OFF."
...
- **`apollo/handlers/done.py`** — the only caller of `retired rerun input builder` in this unit; refactored, proven by route tests.
...
- **WU-5B3a-1 (future, NOT this unit)** — the real consumer of all five columns + `retired rerun input builder` + `read_node_graded_at`. The frozen signatures section is its contract.
...
- **The `done.py` extraction mechanics** (which exact lines move) — allowed to vary as long as: (a) `_find_problem_payload` is NOT duplicated, (b) the live `student_graph`/`old_rubric` stay the live values, (c) `problem_payload` flows through `retired rerun input builder` keyed on `attempt.problem_id`, and (d) both route tests stay byte-identical green.
