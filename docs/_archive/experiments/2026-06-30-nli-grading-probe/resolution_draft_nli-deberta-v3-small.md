...
|---------|----------|--------|-------|-----------|--------|-------------|
| bernoulli_01 | 5 | 0.7500 | 0.2500 | 2 | False | 0 |
...
| b_nli_simp | simp | `simp.horizontal_simplification` | the conduit has no elevation change so height is the same at… | 0.537 | Y | unresolved→– | nli→simp.horizontal_simplific | 0.9505 | 0.0008 | entailment | NLI scores PASS threshold (ent=0.9505>=0.87, con=0.0008<=0.1) |
...
- **b_nli_simp** [NLI] floor=PASS recovered=True false_credit=False veto=False
  ...
  - Scores: ent=0.9505 con=0.0008 neu=0.0488 → label=**entailment**
  - Notes: NLI scores PASS threshold (ent=0.9505>=0.87, con=0.0008<=0.1)
...
| ea_nli_proc | proc | `proc.compute_net_exports` | take the gap between foreign sales and purchases to find the… | 0.449 | Y | unresolved→– | unresolved→– | 0.0306 | 0.6268 | contradiction | NLI scores BELOW threshold (ent=0.0306 need>=0.87; con=0.6268 need<=0.1) |
...
- **ea_nli_proc** [NLI] floor=PASS recovered=False false_credit=False veto=False
  ...
  - Scores: ent=0.0306 con=0.6268 neu=0.3427 → label=**contradiction**
  - Notes: NLI scores BELOW threshold (ent=0.0306 need>=0.87; con=0.6268 need<=0.1)
...
| node_id | type | intended_key | student_surface (truncated) | TSR-vs-display | floor | OFF method→key | ON method→key | NLI ent | NLI con | label | notes |
|---------|------|-------------|----------------------------|---------------|-------|----------------|---------------|---------|---------|-------|-------|
| eb_nli_def | def | `def.real_basis` | real GDP strips out inflation so it captures only the volume… | 0.600 | Y | unresolved→– | unresolved→– | 0.0085 | 0.9868 | contradiction | NLI scores BELOW threshold (ent=0.0085 need>=0.87; con=0.9868 need<=0.1) |
| eb_nli_proc | proc | `proc.compute_real_change` | find the gap between the later and earlier output figures to… | 0.650 | Y | unresolved→– | unresolved→– | 0.0124 | 0.0043 | neutral | NLI scores BELOW threshold (ent=0.0124 need>=0.87; con=0.0043 need<=0.1) |
...
- **eb_nli_def** [NLI] floor=PASS recovered=False false_credit=False veto=False
  ...
  - Scores: ent=0.0085 con=0.9868 neu=0.0047 → label=**contradiction**
  - Notes: NLI scores BELOW threshold (ent=0.0085 need>=0.87; con=0.9868 need<=0.1)
- **eb_nli_proc** [NLI] floor=PASS recovered=False false_credit=False veto=False
  ...
  - Scores: ent=0.0124 con=0.0043 neu=0.9833 → label=**neutral**
  - Notes: NLI scores BELOW threshold (ent=0.0124 need>=0.87; con=0.0043 need<=0.1)
