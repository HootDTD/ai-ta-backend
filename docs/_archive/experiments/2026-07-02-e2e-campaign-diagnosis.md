...
**Corrections (2026-07-04):** four diagnosis labels in this document were later found to be wrong and are corrected inline below — G4.1 causality (PR #86), the D1 empty-bank "abstention" claim (PR #85), the O2 422 label (`docs/_archive/experiments/2026-07-03-weekend-b-o2-422-triage.md`), and the Q1 root cause (PR #94, `docs/_archive/experiments/2026-07-03-weekend-b-q1-verification.md`). The original text is preserved for the record.
...
- **Observed defect that revealed it:** the grader treats an empty `apollo_misconceptions` bank as a reason to **abstain from grading entirely** (`misconception_bank_empty` on 36/36 F1b attempts). Under the emergent design, an empty bank is the *normal cold-start state of every class* — abstaining on it is wrong. Coverage grading and misconception detection are separable signals; the artifact schema already separates them. *(Corrected 2026-07-04: the "abstain from grading entirely" claim was wrong — the `misconception_bank_empty` reason string never set `abstained=True`; it only polluted the abstention reason histograms. PR #85 removed the reason string entirely and replaced it with the explicit `abstention.misconceptions_status` marker, so a cold-start empty bank grades coverage normally with a machine-readable "no misconceptions asserted (empty bank)" annotation.)*
...
1. **Missing `variable_mapping`:** minted problems lack the payload `run_graph_simulation` assumes exists on every problem (seeded subjects always have it) → the G3 crash. Either mint must produce it or the grader must tolerate its absence. *(Corrected 2026-07-04 — causality was inverted: the WU-AAS mint DOES emit `variable_mapping`; the defect was consumer-side — the entity map assembled for `run_graph_simulation` omitted the field. PR #86 fixed both ends of the contract: mint emission verified plus graph-sim tolerance for absence.)*
...
S5: 0 assertions / 38 attempts (vacuous both runs — F1b from the empty bank, F1c *with* bank seeded). All three misconception personas (attempts 3, 4, 11 in the packets) taught their scripted misconception verbatim (bank `trigger_phrases` used verbatim in the beats) and received scorecards with **empty `watch_out`** — graded Proficient/Developing with no warning. Detection appears gated behind resolution success (G1) and/or the D1 abstention logic. Whatever the mechanism: the signal the classroom projection and the pedagogy most depend on is currently silent. **Owner:** resolution/misconception matching; blocked partially by G1/D1. *(Corrected 2026-07-04 — the "gated behind resolution success / D1 abstention" hypothesis was wrong; the actual root cause is a key-prefix mismatch in the candidate-assembly seam: the bank seeder strips the `misc.` prefix when writing the DB `code` column, `candidate_assembly` read the code back unprefixed, and `is_misconception_key()` keys on the prefix — so misconception detection was structurally unreachable regardless of resolver recall. Fixed in PR #94; evidence: `docs/_archive/experiments/2026-07-03-weekend-b-q1-verification.md`.)*
...
torch on Windows requires the pinned `2.6.0+cpu` venv (README'd); Supabase analytics container can't run on Windows Docker Desktop (disabled in harness config); two isolated 422 `malformed_equation`/chat errors and one persona 422 remain unexplained per-attempt failures worth a look (~3/38 ≈ 8% attempt attrition). *(Corrected 2026-07-04 — the `malformed_equation` label was a misattribution: `malformed_equation` is raised only by the SymPy solver on the Done path, which is unreachable from `/chat`; the chat-route 422s are `parser_could_not_extract` (vague-persona word-salad extracting zero nodes). The ~8% attempt attrition is harness-side, not product-side — the campaign driver's broad `try/except` around `raise_for_status()` records the whole attempt as errored and discards the good turns. Evidence: `docs/_archive/experiments/2026-07-03-weekend-b-o2-422-triage.md`.)*
...
1. **[D1] Redesign misconception store** as emergent per-class aggregation over canonical artifacts (brainstorm first — trust-gradient pipeline decision pending); remove abstain-on-empty-bank as part of it. *(Corrected 2026-07-04: there was never an abstain-on-empty-bank — see the D1 correction above; the reason-string pollution was removed in PR #85.)*
...
5. **[G4] WU-AAS mint compatibility:** `variable_mapping` contract, parser tolerance (chained eq / `^` / `x`), dedup at mint, page-level OCR capture for auditability. *(2026-07-04: `variable_mapping` contract closed by PR #86 — consumer-side omission, not a mint gap.)*
6. **[Q1] Misconception detection** re-verified non-vacuous once G1/D1 land (S5 gets its first real precision number). *(Corrected 2026-07-04: Q1 was independent of G1/D1 — `misc.` key-prefix mismatch, fixed in PR #94; see the Q1 correction above.)*
