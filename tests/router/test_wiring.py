...
def test_bundle_from_cache_preserves_citation_contract():
    ...
    bundle = wiring.bundle_from_cache(
        cached, q_effective="why does the p-series converge?",
        subject="Calculus", reason="cyu reply",
    )
...
def test_merge_augment_dedupes_and_caps(monkeypatch):
    ...
    fresh = wiring.bundle_from_cache(  # convenient way to build a bundle
        CachedBundle(snippets=[_snippet(3), _snippet(4)], scoring={},
                     visible_docs_hash="h1", saved_turn=1),
        q_effective="q", subject="Calculus", reason="",
    )
    merged = wiring.merge_augment_bundle(
        cached, fresh, q_effective="q", subject="Calculus", reason="follow-up",
    )
...
def test_scoring_rows_extracted_from_provenance():
    bundle = wiring.bundle_from_cache(
        _cached([1]), q_effective="q", subject="s", reason="",
    )
    bundle.provenance["citation_rankings"] = [
        _scoring_row(1, 0.7), {"no_snippet_id": True}, "garbage",
    ]
...
def test_none_mode_skips_scoring_pool_entirely(monkeypatch):
    ...
    cached = _cached([1, 2], scores={1: 0.8, 2: 0.6})
    bundle = wiring.bundle_from_cache(
        cached, q_effective="why?", subject="Calculus", reason="cyu",
    )
...
async def test_fresh_then_none_lifecycle(db_session, monkeypatch):
    space = SearchSpace(
        name="Wiring test space", slug="wiring-test", subject_name="Calculus",
    )
    ...
    ctx1 = await wiring.prepare_router_context(
        chat_id=chat.chat_id, user_id=chat.user_id,
        search_space_id=space.id, question="What is a p-series?",
    ...
    bundle1 = wiring.bundle_from_cache(
        CachedBundle(snippets=[_snippet(chunk.id)], scoring={},
                     visible_docs_hash="", saved_turn=0),
        q_effective="What is a p-series?", subject="Calculus", reason="",
    )
    bundle1.provenance["citation_rankings"] = [_scoring_row(chunk.id, 0.8)]
    await wiring.persist_turn_outcome(
        chat_id=chat.chat_id, user_id=chat.user_id, ctx=ctx1,
        bundle=bundle1, question="What is a p-series?",
        latency_retrieval_ms=100, latency_answer_ms=200,
    )

    # Turn 2: cache present → fake router says NONE → cached bundle returned
    ctx2 = await wiring.prepare_router_context(
        chat_id=chat.chat_id, user_id=chat.user_id,
        search_space_id=space.id, question="B",
        has_attachments=False,
    )
    ...
    ctx3 = await wiring.prepare_router_context(
        chat_id=chat.chat_id, user_id=chat.user_id,
        search_space_id=space.id, question="what is this?",
    ...
    rows = (
        (
            await db_session.execute(
                select(ChatRouterDecision).where(
                    ChatRouterDecision.chat_session_id == chat.id
                )
            )
        )
        .scalars()
