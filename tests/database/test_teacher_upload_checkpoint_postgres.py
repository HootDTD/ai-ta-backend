...
from database.models import (
    AITAChunk, AITADocument, DocumentStatus, SearchSpace,
    TeacherCourse, TeacherUpload, TeacherUploadJob,
)
from indexing.connector_document import AITAConnectorDocument
from knowledge.teacher_weekly import (
    TeacherWeeklyStorage, ClaimedUploadJob,
    JOB_STATE_PROCESSING, UPLOAD_STATUS_PROCESSING,
)
...
class _NoopStorage:
    ...
    def download_bytes(self, **k): return b""
...
async def _seed(db_session):
    ...
    upload = TeacherUpload(
        search_space_id=space.id, week=0, kind="textbook", title="Textbook",
        source_name="t.pdf", status=UPLOAD_STATUS_PROCESSING, storage_key="k",
        started_at=datetime.now(UTC),
        artifact_manifest={"bucket": "b", "pages": [],
                           "embed_progress": {"last_completed_page": 1}},  # resume from page 1
    )
    db_session.add(upload)
    await db_session.flush()
    job = TeacherUploadJob(
        upload_id=upload.id, state=JOB_STATE_PROCESSING, lease_owner="w1",
...
async def test_index_existing_upload_resumes_and_finalizes(db_session, monkeypatch):
    ...
    monkeypatch.setattr("indexing.checkpoint_indexer.embed_texts",
                        lambda texts: [fake_embedding(t) for t in texts])
    monkeypatch.setattr("indexing.document_embedder.embed_text",
                        lambda text, **k: fake_embedding(text))
    # Two pages of chunks; page 1 should be skipped (pointer=1), page 2 embedded.
    chunk_pairs = [("p1", {"page_number": 1, "chunk_type": "body"}),
                   ("p2", {"page_number": 2, "chunk_type": "body"})]
    monkeypatch.setattr("indexing.document_chunker.items_to_chunk_texts",
                        lambda items: chunk_pairs)

    storage = TeacherWeeklyStorage(storage_client=_NoopStorage())
    connector = AITAConnectorDocument(
        title="Textbook", source_markdown="body", unique_id=f"teacher-upload:{upload.id}",
        search_space_id=space.id, material_kind="textbook", week=None,
    )
    claimed = ClaimedUploadJob(
        job_id=job.id, upload_id=upload.id, search_space_id=space.id, week=0,
        kind="textbook", title="Textbook", source_name="t.pdf", storage_key="k",
    )
    await storage._index_existing_upload_async(
        claimed=claimed, connector_doc=connector, items=[],
        ingestion=_FakeIngestion(), source_sha256="sha",
    )
...
async def test_index_existing_upload_resolves_existing_document(db_session, monkeypatch):
    ...
    existing = AITADocument(
        title="Textbook", content="old", search_space_id=space.id,
