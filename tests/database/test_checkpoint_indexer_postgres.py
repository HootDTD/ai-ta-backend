...
def _pairs(pages: list[int]) -> list[tuple[str, dict]]:
    out = []
    for p in pages:
        out.append((f"page-{p}-text", {"page_number": p, "chunk_type": "body",
...
async def _make_document(db_session) -> AITADocument:
    ...
    doc = AITADocument(
        title="Textbook", content="Pending...", content_hash="ckpt-hash",
        unique_identifier_hash="ckpt-uid", search_space_id=space.id,
    )
    db_session.add(doc)
...
async def test_embed_and_persist_is_idempotent_per_page(db_session):
    doc = await _make_document(db_session)
    fn = lambda texts: [fake_embedding(t) for t in texts]
    kwargs = dict(
        session_factory=_session_factory(db_session), document_id=doc.id,
        chunk_pairs=_pairs([1, 2]), after_page=0, batch_size=10, embed_fn=fn,
    )
...
async def test_batch_failure_rolls_back_and_resume_completes(db_session):
    ...
    with pytest.raises(RuntimeError):
        await ci.embed_and_persist_chunks(
            session_factory=_session_factory(db_session),
            document_id=doc.id, chunk_pairs=_pairs([1, 2, 3]),
            after_page=0, batch_size=1, embed_fn=explode_on_second_batch,
        )
    ...
    last = await ci.embed_and_persist_chunks(
        session_factory=_session_factory(db_session),
        document_id=doc.id, chunk_pairs=_pairs([1, 2, 3]),
        after_page=1, batch_size=1, embed_fn=fn,
    )
    assert last == 3
    assert await _count_chunks(db_session, doc.id) == 3
