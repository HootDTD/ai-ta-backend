...
async def _seed_attempts_with_findings(
    ...
    *,
    ...
    for i in range(n_attempts):
        attempt = ProblemAttempt(session_id=session_id, problem_id=problem_code, difficulty="intro")
        db.add(attempt)
        await db.flush()
        run = await _seed_run(db, attempt_id=attempt.id, search_space_id=search_space_id)
...
async def test_sweep_quarantines_concentrated_problem_on_right_bigint_row(db_session):
    ...
    refreshed = (
        await db_session.execute(select(ConceptProblem).where(ConceptProblem.id == p.id))
    ).scalar_one()
...
async def test_sweep_does_not_quarantine_uniform_or_sparse_problem(db_session):
    ...
    await _seed_attempts_with_findings(
        ...
        problem_code="P",
        n_attempts=10,
        miss_plan=lambda i: (["p_bad"] if i < 9 else []) + (["p_a"] if i == 0 else []),
    )
    ...
    await _seed_attempts_with_findings(
        ...
        problem_code="Q",
        n_attempts=10,
        miss_plan=lambda i: (["q_x"] if i < 8 else []) + (["q_y"] if i < 8 else []),
    )
    ...
    await _seed_attempts_with_findings(
        ...
        problem_code="R",
        n_attempts=4,
        miss_plan=lambda i: ["r_bad"] if i < 4 else [],
    )
    ...
    for row in (p, q, r):
        refreshed = (
            await db_session.execute(select(ConceptProblem).where(ConceptProblem.id == row.id))
        ).scalar_one()
...
async def test_sweep_is_course_scoped_by_run_search_space_id(db_session):
    ...
    cid_a = await seed_concept(
        db_session, search_space_id=sid_a, subject_slug="s_a", concept_slug="c_a"
    )
    cid_b = await seed_concept(
        db_session, search_space_id=sid_b, subject_slug="s_b", concept_slug="c_b"
    )
    row_a = await _seed_problem_row(
        db_session, concept_id=cid_a, search_space_id=sid_a, problem_code="SHARED"
    )
    row_b = await _seed_problem_row(
        db_session, concept_id=cid_b, search_space_id=sid_b, problem_code="SHARED"
    )
    ...
    await _seed_attempts_with_findings(
        ...
        problem_code="SHARED",
        n_attempts=10,
        miss_plan=lambda i: (["bad_a"] if i < 9 else []) + (["low_a"] if i == 0 else []),
    )
    await _seed_attempts_with_findings(
        db_session,
        ...
        problem_code="SHARED",
        n_attempts=10,
        miss_plan=lambda i: (["bad_b"] if i < 9 else []) + (["low_b"] if i == 0 else []),
    )
    ...
    assert report.n_quarantined == 1
    refreshed_a = (
        await db_session.execute(select(ConceptProblem).where(ConceptProblem.id == row_a.id))
    ).scalar_one()
    refreshed_b = (
        await db_session.execute(select(ConceptProblem).where(ConceptProblem.id == row_b.id))
    ).scalar_one()
...
async def test_sweep_reclears_when_concentration_no_longer_fires(db_session):
    ...
    sid = await seed_search_space(db_session)
    cid = await seed_concept(
        db_session, search_space_id=sid, subject_slug="s_rc", concept_slug="c_rc"
    )
    p = await _seed_problem_row(
        db_session, concept_id=cid, search_space_id=sid, problem_code="P-RC"
    )
    ...
    await _seed_attempts_with_findings(
        ...
        problem_code="P-RC",
        n_attempts=10,
        miss_plan=lambda i: (["x"] if i < 8 else []) + (["y"] if i < 8 else []),
    )
...
async def test_sweep_missing_node_filter_is_load_bearing(db_session):
    ...
    sid = await seed_search_space(db_session)
    cid = await seed_concept(
        db_session, search_space_id=sid, subject_slug="s_mn", concept_slug="c_mn"
    )
    p = await _seed_problem_row(
        db_session, concept_id=cid, search_space_id=sid, problem_code="P-MN"
    )
    sess = await _shared_session(db_session, search_space_id=sid, concept_id=cid)
...
async def test_sweep_ignores_empty_reference_node_ids_findings(db_session):
    ...
    sid = await seed_search_space(db_session)
    cid = await seed_concept(
        db_session, search_space_id=sid, subject_slug="s_nl", concept_slug="c_nl"
    )
    p = await _seed_problem_row(
        db_session, concept_id=cid, search_space_id=sid, problem_code="P-NULL"
    )
    sess = await _shared_session(db_session, search_space_id=sid, concept_id=cid)
    for i in range(10):
...
async def test_sweep_emits_audit_log_on_fire(db_session, caplog):
    ...
    sid = await seed_search_space(db_session)
    cid = await seed_concept(
        db_session, search_space_id=sid, subject_slug="s_lg", concept_slug="c_lg"
    )
    ...
    await _seed_attempts_with_findings(
        ...
        problem_code="P-LOG",
        n_attempts=10,
        miss_plan=lambda i: (["bad"] if i < 9 else []) + (["low"] if i == 0 else []),
    )
...
async def test_sweep_emits_clear_log_on_reclear(db_session, caplog):
    ...
    sid = await seed_search_space(db_session)
    cid = await seed_concept(
        db_session, search_space_id=sid, subject_slug="s_cl", concept_slug="c_cl"
    )
    p = await _seed_problem_row(
        db_session, concept_id=cid, search_space_id=sid, problem_code="P-CL"
    )
    from datetime import UTC, datetime
...
async def test_sweep_idempotent_second_run_no_change(db_session):
    ...
    sid = await seed_search_space(db_session)
    cid = await seed_concept(
        db_session, search_space_id=sid, subject_slug="s_id", concept_slug="c_id"
    )
    p = await _seed_problem_row(
        db_session, concept_id=cid, search_space_id=sid, problem_code="P-ID"
    )
    sess = await _shared_session(db_session, search_space_id=sid, concept_id=cid)
    await _seed_attempts_with_findings(
        db_session,
...
async def test_sweep_resolution_miss_is_logged_and_skipped(db_session, caplog):
    ...
    sid = await seed_search_space(db_session)
    cid = await seed_concept(
        db_session, search_space_id=sid, subject_slug="s_or", concept_slug="c_or"
    )
    ...
    await _seed_attempts_with_findings(
        ...
        problem_code="ORPHAN",
        n_attempts=10,
        miss_plan=lambda i: ["bad"] if i < 9 else [],
    )
    ...
    assert report.n_quarantined == 0
    misses = [
        r for r in caplog.records if getattr(r, "event", None) == "quarantine_resolution_miss"
    ]
    assert len(misses) == 1
...
async def test_sweep_clamps_multiple_findings_per_attempt_for_same_node(db_session, caplog):
    ...
    sid = await seed_search_space(db_session)
    cid = await seed_concept(
        db_session, search_space_id=sid, subject_slug="s_cp", concept_slug="c_cp"
    )
    p = await _seed_problem_row(
        db_session, concept_id=cid, search_space_id=sid, problem_code="P-DUP"
    )
    sess = await _shared_session(db_session, search_space_id=sid, concept_id=cid)
    for i in range(10):
...
async def test_quarantined_tier2_problem_excluded_by_selector(db_session):
    ...
    sid = await seed_search_space(db_session)
    cid = await seed_concept(
        db_session, search_space_id=sid, subject_slug="s_sel", concept_slug="c_sel"
    )
    ...
    db_session.add(
        ConceptProblem(
            concept_id=cid,
            problem_code=live_payload["id"],
            difficulty=live_payload["difficulty"],
    ...
    db_session.add(
        ConceptProblem(
            concept_id=cid,
            problem_code=quarantined_payload["id"],
            difficulty=quarantined_payload["difficulty"],
            payload=quarantined_payload,
            tier=2,
            search_space_id=sid,
    ...
    assert quarantined_payload["id"] not in returned_ids, (
        "quarantined problem must NOT be selectable"
    )
...
async def test_live_tier2_problem_still_selectable(db_session):
    ...
    sid = await seed_search_space(db_session)
    cid = await seed_concept(
        db_session, search_space_id=sid, subject_slug="s_lv", concept_slug="c_lv"
    )
    db_session.add(
        ConceptProblem(
            concept_id=cid,
            problem_code=payload["id"],
...
async def test_shadow_findings_feed_sweep_end_to_end(db_session, monkeypatch):
    ...
    cp_row = (
        await db_session.execute(select(ConceptProblem).where(ConceptProblem.problem_code == code))
    ).scalar_one()
    cp_row.search_space_id = sid
    await db_session.flush()
    sess, attempt = await _seed_shadow_session(db_session, current_code=code, sid=sid, cid=cid)

    await _run_shadow_done(db_session, sess.id, monkeypatch, neo_patches=_neo_stubs(attempt.id))
    ...
    assert prod_finding.reference_node_ids, (
        "production missing_node finding must carry reference_node_ids"
    )
    ...
    await _seed_attempts_with_findings(
        ...
        problem_code=code,
        n_attempts=9,
        miss_plan=lambda i: [prod_node_key[0]],
    )
