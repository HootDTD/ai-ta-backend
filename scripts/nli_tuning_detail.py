...
def recall_breakpoints(data: dict, max_con: float) -> list[tuple]:
    ...
    rows = sorted(
        [
            (me, rec, tp, fp)
            for (me, mc, prec, rec, tp, fp, fn) in data["reference"]["fine_grid"]
...
A("")
A(
    "Complete cell-by-cell grids are in `fine_grid_<model>.csv` (50 `min_ent` × 7 "
...
for tag, data in (("small", SMALL), ("large", LARGE)):
    A(
        f"### 7.{'1' if tag == 'small' else '2'} {tag} — recall breakpoints (`max_con={MAX_CON_REPORT}`)"
    )
    A("")
    A("| `min_ent` range | recall | tp/56 |")
    A("|---|---|---|")
    for lo, hi, rec, tp, _fp in recall_breakpoints(data, MAX_CON_REPORT):
...
for tag, data, me in (("small", SMALL, 0.50), ("large", LARGE, 0.90)):
    for mc, rec, prec, _tp, _fp in maxcon_sensitivity(data, me):
        cap = "∞" if mc >= 1.0 else f"{mc:.2f}"
        A(f"| {tag} | {me:.2f} | {cap} | {rec:.3f} | {prec:.3f} |")
A("")
A(
    "`max_con` behaves differently per model. For **large** it is inert — recall is "
...
A("")
A(
    "Every positive NOT credited at each model's **max-recall** operating point "
...
for tag, data, me in (("small", SMALL, 0.50), ("large", LARGE, 0.90)):
    ...
    A(
        f"### 8.{'1' if tag == 'small' else '2'} {tag} — {len(misses)}/56 missed (at `min_ent={me:.2f}`)"
    )
    A("")
...
A("")
A(
    "All 24 veto records with each model's **max entailment over the concept's "
...
for sr, lr in zip(srecs, lrecs, strict=True):
    ...
    A(
        f"| {kind} | `{sr['code']}` | {sr['_best_ent']:.3f} | {s_fire} | "
        f"{lr['_best_ent']:.3f} | {l_fire} | {surf[:42]} |"
    )
A("")
A(
    "Note the `neg` rows scoring 0.88–0.98 (e.g. a correct deflation statement "
