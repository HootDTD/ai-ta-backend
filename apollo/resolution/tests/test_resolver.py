...
def test_polar_near_miss_resolves_to_misconception_through_resolve_attempt():
    ...
    misc = _cand(
        "misc.pressure_velocity_same_direction", node_type="definition",
        is_misc=True,
    ...
    ref_alone = resolve_attempt(
        KGGraph(nodes=[_node("s1", "definition",
                             {"concept": "pressure", "meaning": meaning})]),
        (ref,),
    )
    ...
    assert by_key["def.pressure_velocity_tradeoff"] > by_key[
        "misc.pressure_velocity_same_direction"
    ]
...
def test_exact_alias_and_fuzzy_dual_hit_resolves_via_alias_not_fuzzy():
    ...
    graph = KGGraph(nodes=[
        _node("s1", "definition",
              {"concept": "pressure", "meaning": "density is constant"}),
    ])
    # surface == 'pressure density is constant'; the alias matches it verbatim.
    cand = _cand("def.x", node_type="definition",
                 aliases=("pressure density is constant",))
