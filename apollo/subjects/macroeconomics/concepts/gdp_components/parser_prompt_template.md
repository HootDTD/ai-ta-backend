...
For type=equation: set "symbolic" (a parseable string using the canonical
macroeconomic symbols as underscore-free identifiers — e.g. GDP, C, INV, G, X,
M, NX, GNP, NNP, DEP, RIN, ROUT; write INV for investment, never the bare
letter I; use Rational(1,2) for halves, ** for exponents, avoid unicode; prefer
zero-form LHS - (RHS), e.g. the GDP identity as "GDP - (C + INV + G + NX)"),
"label" (short human name from what the student called it, e.g. "GDP expenditure
identity", "net exports", "net national product"), and optionally "variables"
(the list of symbols). Leave the rest null.

For type=condition: set "applies_when" (natural language, e.g. "only final goods
and services count", "imports exceed exports so net exports are negative") and
"label".
For type=simplification: set "applies_when" and "transformation".
For type=definition: set "concept" and "meaning" (e.g. concept "depreciation",
meaning "capital worn out over the year, subtracted from GNP to get NNP").
For type=variable_mapping: set "term" and "symbol" (e.g. term "government
purchases", symbol "G").
For type=procedure_step: set "action" (natural-language description of what the
student does at this stage, e.g. "subtract imports from exports to get net
exports", "add C, INV, G, and NX to obtain GDP") and "purpose" (why this step is
done).
...
NOT when describing what an economy or a national account definitionally IS.
First-person framing ("first I would...", "I'll add up...", "then I subtract...")
or explicit step numbering ("step 1:...", "next, compute...") marks plan-speak.
Definitional or descriptive statements ("GDP is the total of consumption,
investment, government spending, and net exports") are NOT plan-speak and must
NOT produce procedure_step entries. One procedure_step per planned action, in the
order the student stated them. Procedure steps in the same response are implicitly
chained in order — do not emit separate "next" or "after" markers.
...
- SCOPES: condition -> equation, OR simplification -> equation
  (the condition/simplification governs when/how that equation applies, e.g. the
  final-goods-only rule scopes the GDP expenditure identity).
- DEPENDS_ON: any -> any except self-loops (one entry relies on another, e.g.
  the NNP equation depends on the GNP equation, a step depends on a condition).
...
- explicit: the student's wording directly states the relation ("use the GDP
  identity to add up the components" -> USES is explicit; "first compute net
  exports, then sum the components" -> PRECEDES is explicit).
...
- 1.0: the student wrote it explicitly and unambiguously (math symbols, exact
  named identity, verbatim formula).
...
Rules:
- Return ONLY what the student explicitly said. Do NOT add economics the student did not mention.
- If the student said nothing extractable, return {"entries": [], "edges": []}.
- Do not correct the student. If they said an identity wrong, extract it as stated.
