...
For type=equation: set "symbolic" (a parseable string using the macro symbols
the student used as underscore-free identifiers — e.g. nomGDP for nominal GDP,
realGDP for real GDP, deflator for the GDP deflator, PI for the price index,
growth for a percentage growth rate; use Rational(1,2) for halves, ** for
exponents, avoid unicode; prefer zero-form LHS - (RHS), e.g.
"deflator - (nomGDP/realGDP)*100"), "label" (short human name from what the
student called it, e.g. "GDP deflator definition" or "growth rate"), and
optionally "variables" (the list of symbols). Leave the rest null.

For type=condition: set "applies_when" (natural language) and "label".
For type=simplification: set "applies_when" and "transformation" (e.g. the
student treats the quoted price index as the GDP deflator so the deflator
definition can be solved for real GDP).
For type=definition: set "concept" and "meaning" (e.g. concept "real GDP",
meaning "GDP adjusted for inflation / valued at constant base-year prices").
For type=variable_mapping: set "term" and "symbol" (e.g. term "nominal GDP",
symbol "nomGDP").
For type=procedure_step: set "action" (natural-language description of what the
student does at this stage — e.g. "rearrange the deflator definition to solve
for real GDP and substitute the given values") and "purpose" (why this step is
done — e.g. "deflate nominal GDP by the price index to get real GDP").
...
NOT when describing what an economy or a price level physically does. First-person
framing ("first I would...", "I'll rearrange...", "then I divide...") or explicit
step numbering ("step 1:...", "next, solve for...") marks plan-speak. Descriptive
economic narration ("when prices rise, nominal GDP overstates output") is NOT
plan-speak and must NOT produce procedure_step entries. One procedure_step per
planned action, in the order the student stated them. Procedure steps in the same
response are implicitly chained in order — do not emit separate "next" or "after"
markers.
...
- explicit: the student's wording directly states the relation ("use the
  deflator equation to find real GDP" -> USES is explicit; "first deflate, then
  compute the growth rate" -> PRECEDES is explicit).
- inferred: you are connecting two things the student mentioned separately but
  did NOT explicitly relate (e.g. a late simplification you SCOPES-link to an
...
- 1.0: the student wrote it explicitly and unambiguously (math symbols, exact
  named formula, verbatim definition).
...
- Do not correct the student. If they stated an equation or definition wrong, extract it as stated.
