...
def test_formula_is_weighted_sum_minus_penalty():
    w = CompositeWeights(w_n=0.6, w_e=0.25, p=0.15)
    assert composite_score(1.0, 1.0, 0.0, w) == 0.85
    assert composite_score(0.5, 0.4, 1.0, w) == round(0.6 * 0.5 + 0.25 * 0.4 - 0.15 * 1.0, 6)
...
def test_default_weights_match_spec():
    w = load_weights()
    assert w == CompositeWeights(w_n=0.6, w_e=0.25, p=0.15)
