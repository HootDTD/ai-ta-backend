...
def test_ordinal_ref_resolves_to_this_response_node():
    eq = _eq()
    ctx = GraphContext(nodes=(
        ContextNode(node_id="eq_prev", node_type="equation", label="prev"),
    ))
...
def test_safe_context_id_never_collides_with_ordinal():
    graph = KGGraph(nodes=[
        _eq("stu_eq000000000"),
        build_node(
            node_type="procedure_step", node_id="stu_step00000000",
            attempt_id=-1, source="parser",
...
def test_ordinal_miss_returns_none_not_context():
    ...
    eq = _eq()
    ctx = GraphContext(nodes=(
        ContextNode(node_id="n9", node_type="equation", label="trap"),
    ))
