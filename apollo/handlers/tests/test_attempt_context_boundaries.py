...
async def _seed(db: AsyncSession, *, phase: str):
    ...
    attempt = ProblemAttempt(
        session_id=session.id, problem_id="p1", difficulty="intro",
    )
    db.add(attempt)
    await db.commit()
...
async def test_next_problem_clears_session_scoped_attempt_context(db):
    ...
    problem = SimpleNamespace(
        id="p2", concept_id=1, difficulty="intro", problem_text="new",
        given_values={}, target_unknown="x",
    )
    ...
    with patch(
        ...
        response = await handle_next(
            db=db, session_id=session.id, difficulty="intro",
        )
...
async def test_restart_problem_clears_session_scoped_attempt_context(db):
    ...
    with patch(
        ...
        response = await handle_restart_problem(
            db=db, neo=object(), session_id=session.id,
        )
