...
async def test_restart_wipes_kg_and_messages_for_current_attempt(db_with_report_session):
    ...
    attempt.result = None
    s.add_all([
        KGEntry(session_id=session_id, attempt_id=attempt.id, type="equation",
                content={"symbolic": "x - 1", "label": "to_be_wiped"}, source="parser"),
        Message(session_id=session_id, attempt_id=attempt.id, role="student", content="hi", turn_index=0),
    ])
    await s.commit()
    ...
    kg_rows = (await s.execute(
        select(KGEntry).where(KGEntry.attempt_id == attempt.id)
    )).scalars().all()
    msg_rows = (await s.execute(
        select(Message).where(Message.attempt_id == attempt.id)
    )).scalars().all()
    assert kg_rows == []
    assert msg_rows == []

    attempt_after = (await s.execute(
...
async def test_restart_does_not_touch_other_attempts(db_with_report_session):
    ...
    await s.flush()
    s.add_all([
        KGEntry(session_id=session_id, attempt_id=other.id, type="equation",
                content={"symbolic": "survivor - 0"}, source="parser"),
        KGEntry(session_id=session_id, attempt_id=current.id, type="equation",
    ...
    survivors = (await s.execute(
        select(KGEntry).where(KGEntry.attempt_id == other.id)
    )).scalars().all()
    assert len(survivors) == 1
    victims = (await s.execute(
        select(KGEntry).where(KGEntry.attempt_id == current.id)
    )).scalars().all()
    assert victims == []
