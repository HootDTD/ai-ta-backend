...
def build_s2_raw() -> list[dict]:
    ...
    provisioning (set_id=2 promoted Problem 1(a); set_id=3 promoted Problem
    1(b); set_id=1 was the first, fully-rejected attempt kept as an ingest
    finding, not fed to the judge as a pass/fail item)."""
    tmp_dir = Path(r"C:\Users\ultra\AppData\Local\Temp")
    final2 = json.loads((tmp_dir / "authored_set_final2.json").read_text())
    final3 = json.loads((tmp_dir / "authored_set_final3.json").read_text())

    items = []
    for set_id, resp in ((2, final2), (3, final3)):
